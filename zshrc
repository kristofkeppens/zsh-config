...
PATH='/bin:/sbin:/usr/bin:/usr/sbin:/usr/local/bin:/usr/kerberos/bin'
if [ -d $HOME/bin ]; then
PATH=$PATH:/$HOME/bin
fi
# Set your Time Zone
...
alias buddha='echo -e "${GREEN}"; clear ; fortunes -ao; echo -e "${CYAN}"" Peace be with you $USER" ; echo '

function getcow() {
cows=(beavis.zen bong bud-frogs bunny cheese cower daemon default
dragon dragon-and-cow elephant elephant-in-snake eyes
flaming-sheep ghostbusters head-in hellokitty kiss kitty koala
kosh luke-koala meow milk moofasa moose mutilated ren satanic
sheep skeleton small sodomized stegosaurus stimpy supermilker
surgery telebears three-eyes turkey turtle tux udder vader
vader-koala www );
print $cows[$[${RANDOM}%50+1]];
}
...
echo -e "${GREEN}"; fortune -a | cowsay -f `getcow`
echo -e "${LIGHTYELLOW}";echo " " `date`; echo ""
echo -ne "${LIGHTBLUE} Hello $USER"
echo
echo
